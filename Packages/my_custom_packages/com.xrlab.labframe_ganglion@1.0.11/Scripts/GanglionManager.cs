...
public class GanglionManager : LabSingleton<GanglionManager>, IManager
{
    ...
    public void ReceiveData(string json) // called by Android plugin
    {
        ...
        EEGDataClass _data_1 = new EEGDataClass(values[0], values[2], values[4], values[6]);
        EEGDataClass _data_2 = new EEGDataClass(values[1], values[3], values[5], values[7]);
        if (_doWriteEegData)
        {
            LabDataManager.Instance.WriteData(_data_1);
            LabDataManager.Instance.WriteData(_data_2);
        }
    ...
    /// 更新tag訊息
    /// </summary>
    public void UpdateTag(string _tag)
    {
        tag = _tag;
    }
